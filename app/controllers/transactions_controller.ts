...
export default class TransactionsController {
    ...
    public async index({ response, auth }: HttpContext) {
        try {
            ...
            const query = Transaction.query().preload('donation', (query) => query.preload('user')
        .preload('campaign', (campaignquery) => campaignquery.preload('user')))
            ...
            return response.json({transactions : transactions.map(transaction => ({
                id: transaction.id,
    ...
    public async show({ params, auth, response }: HttpContext) {
        try {
            const user = auth.user
            const transaction = await Transaction.query().where('id', params.id).preload('donation', (query) => query.preload('user')
            .preload('campaign', (campaignquery) => campaignquery.preload('user'))).first()
            if(!transaction){
    ...
    public async update({auth, request, response }: HttpContext) {
        try {
            ...
            const transaction = await Transaction.query().where('orderId', orderId).preload('donation').first()
            const user = auth.user
            if(user){
                return response.status(403).json({ message: 'Forbidden: You are not allowed to update transactions' })
            }
            if(!transaction) {
                return response.status(404).json({ message: 'Transaction not found' })
            }
    ...
    public async destroy({ params, response, auth }: HttpContext) {
        try {
            const user = auth.user
            const transaction = await Transaction.query().where('id', params.id).preload('donation').first()
            if(!transaction) {
                return response.status(404).json({ message: 'Transaction not found' })
            }
            ...
            return response.status(403).json({message: "You are not allowed to delete this transaction"})
        } catch (error) {
            return response.status(500).json({message: "Internal server error"})
        }
    }
